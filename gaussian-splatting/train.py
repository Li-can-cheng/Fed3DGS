...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if isinstance(gt_image, torch.Tensor):
            ...
            gt_image = PILtoTorch(Image.open(gt_image), (viewpoint_cam.image_width, viewpoint_cam.image_height)).cuda()[
                       :3, ...]
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                            testing_iterations, scene, render, (pipe, -1, background))
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if isinstance(viewpoint.original_image, torch.Tensor):
                        ...
                        gt_image = torch.clamp(PILtoTorch(Image.open(viewpoint.original_image),
                                                          (viewpoint.image_width, viewpoint.image_height)).cuda()[:3,
                                               ...], 0.0, 1.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[30_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[30_000, 30_000])
